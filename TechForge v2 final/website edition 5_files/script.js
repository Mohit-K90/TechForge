...
// Create an empty array to store the selected parts
var selectedParts = [];

// Function to add a selected part to the array
function selectComponent(category, partName) {
  var selectedPart = {
    category: category,
    name: partName,
  };
...
// Function to update the displayed list of selected parts
function updateSelectedPartsList() {
  var selectedPartsList = document.getElementById('selectedPartsList');

  // Clear the existing list
  selectedPartsList.innerHTML = '';

  // Iterate through the selected parts and display them
  for (var i = 0; i < selectedParts.length; i++) {
    var part = selectedParts[i];
    var listItem = document.createElement('li');
    listItem.textContent = part.category + ': ' + part.name;
    selectedPartsList.appendChild(listItem);
  }
}

// Call the updateSelectedPartsList function initially
updateSelectedPartsList();
